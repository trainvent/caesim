...
- SUPABASE_ANON_KEY or SUPABASE_KEY (publishable key) — used for auth lookups
- SUPABASE_SERVICE_ROLE_KEY or SUPABASE_SECRET_KEY (service role) — required for upserts
- CAESIM_CREDIT_ADMIN_TOKEN — an admin token used by the CLI/CI to perform grants
...
```bash
supabase secrets set SUPABASE_SECRET_KEY="$SUPABASE_SERVICE_ROLE_KEY" SUPABASE_KEY="$SUPABASE_PUBLISHABLE_KEY" CAESIM_CREDIT_ADMIN_TOKEN="$ADMIN_TOKEN"
```
...
CAESIM_CREDIT_ADMIN_TOKEN=<admin token for grant>
```
...
- The CLI now supports delegating balance and consume operations to this gateway when `CAESIM_CREDIT_GATEWAY_URL` is set.
